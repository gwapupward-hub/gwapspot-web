...
export function ReceiptAnalytics({ code, theme }: Props) {
  useEffect(() => {
    track("receipt_viewed", code, theme);
  }, [code, theme]);
  return null;
}
...
export function ReceiptCta({ code, theme, href, children, primary = false }: Props & { href: string; children: ReactNode; primary?: boolean }) {
  return (
    <a
      href={href}
      onClick={() => track("receipt_cta_clicked", code, theme)}
      style={primary
        ? { background: "var(--proof-accent)", color: "#050505", padding: "13px 18px", borderRadius: 12, fontWeight: 800, textDecoration: "none" }

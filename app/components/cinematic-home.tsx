    ...
    <main className="cinematic-home">
      ...
      <header className={`cinematic-nav${scrolled ? " is-scrolled" : ""}`}>
        ...
        <div className="cinematic-nav-actions">
          <button className="icon-button" type="button" aria-label="Search GWAP products" onClick={() => setSearchOpen(true)}><Icon name="search" /></button>
      ...
      {searchOpen ? (
        <div className="search-overlay" role="dialog" aria-modal="true" aria-label="Search the GWAP ecosystem" onMouseDown={(event: ReactMouseEvent<HTMLDivElement>) => { if (event.target === event.currentTarget) setSearchOpen(false); }}>
          <div className="search-panel">
            <div className="search-field"><Icon name="search" /><input ref={searchInputRef} value={query} onChange={(event: ChangeEvent<HTMLInputElement>) => setQuery(event.target.value)} placeholder="Search GNS, GwapScore, DIMI…" /><button type="button" onClick={() => setSearchOpen(false)} aria-label="Close search"><Icon name="close" /></button></div>

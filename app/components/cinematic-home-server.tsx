...
import {
  ...
  isExternalProductDestination,
  socialLinks,
} from "../lib/ecosystem";
import { getRoadmapPhaseClass, roadmapPhases } from "../lib/roadmap";
import { CinematicNavigation } from "./cinematic-navigation";
import { CountUp } from "./count-up";
import { GwapEcosystemGraph } from "./gwap-ecosystem-graph";
import { HomeUtility } from "./home-utility";

type IconName = "arrow" | "spark" | "shield" | "network";
  ...
  const paths: Record<IconName, ReactNode> = {
    ...
    network: (
      <>
        ...
        <path d="m6 8.2 3.4 2M18 8.2l-3.4 2M7.7 17.7l2.5-3.2M16.3 17.7l-2.5-3.2" />
      </>
    ),
  };
...
export function CinematicHomeServer() {
  return (
      ...
      <section className="story-section roadmap-story" id="roadmap" data-story-section>
        ...
        <div className="roadmap-rail">
          {roadmapPhases.slice(0, 3).map((phase) => (
            <article className={`phase-card ${getRoadmapPhaseClass(phase.status)}`} key={phase.phase}>
              <span>{phase.phase.toUpperCase()}</span>
              <div className="phase-orb"><i /></div>
              <h3>{phase.title}</h3>
              <p>{phase.homepageSummary}</p>

...
export async function GET(request: Request) {
  ...
  const subject = `telegram:${account.telegramUserId}`;
  ...
  try {
    ...
    const result = await listDailyIdeaProjects(subject, {
      offset: Number(url.searchParams.get("offset") || 0),
      limit: Number(url.searchParams.get("limit") || 5),
    });
    return json({ contractVersion: DAILY_IDEAS_SERVICE_CONTRACT_VERSION, ...result });
  } catch (error) {
    console.error("daily_ideas_telegram_projects_list_failed", {
      requestId,
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (action === "develop") {
      ...
      console.info("daily_ideas_telegram_project_started", {
        requestId,
        actor: getTelegramActor(account.telegramUserId),
        ideaId,
    ...
    console.error("daily_ideas_telegram_projects_write_failed", {
      requestId,
      actor: getTelegramActor(account.telegramUserId),
      action,
      name: error instanceof Error ? error.name : "Error",

...
export async function GET(request: Request) {
  ...
  try {
    ...
    console.error("daily_ideas_telegram_delivery_read_failed", {
      requestId,
...
export async function POST(request: Request) {
  ...
  try {
    if (body.action === "preferences") {
      ...
      const delivery = await updateDailyIdeasDeliveryPreferences(account.telegramUserId, {
      ...
      console.info("daily_ideas_telegram_delivery_preferences_updated", {
        requestId,
        actor: getTelegramActor(account.telegramUserId),
    ...
    if (body.action === "claim") {
      ...
      const deliveries = await Promise.all(claimed.map(async (delivery) => {
        const preferences = await getDailyIdeasPreferences(`telegram:${delivery.telegramUserId}`);
        const category = preferences.categories.find((item) => item !== "general") || preferences.categories[0] || "general";
        return { ...delivery, category };
      }));
      console.info("daily_ideas_telegram_delivery_claimed", {
        requestId,
    ...
    if (body.action === "complete") {
      const account = accountFrom(body.telegramUserId);
      if (!account || typeof body.deliveryId !== "string" || typeof body.success !== "boolean") {
        return json({ error: "Invalid delivery completion", requestId }, 400);
      }
      const record = await completeDailyIdeasDelivery({
        telegramUserId: account.telegramUserId,
        deliveryId: body.deliveryId,
        success: body.success,
      });
      if (!record) return json({ error: "Unknown delivery", requestId }, 404);
      console.info("daily_ideas_telegram_delivery_completed", {
        requestId,
        actor: getTelegramActor(account.telegramUserId),
        deliveryId: body.deliveryId,
        success: body.success,
    ...
    console.error("daily_ideas_telegram_delivery_failed", {
      requestId,
      action: body.action,
      name: error instanceof Error ? error.name : "Error",
    });

...
export async function GET(request: Request) {
  ...
  try {
    const result = await listSavedDailyIdeas(`telegram:${account.telegramUserId}`, { offset, limit });
    return json({ contractVersion: DAILY_IDEAS_SERVICE_CONTRACT_VERSION, ...result });
  } catch (error) {
    console.error("daily_ideas_telegram_saves_list_failed", {
      requestId,
...
export async function POST(request: Request) {
  ...
  if (!account || !action || !/^[A-Za-z0-9_-]{1,80}$/.test(ideaId)) {
    return json({ error: "Invalid save request", requestId }, 400);
  }
  ...
  try {
    ...
    if (action === "unsave") {
      const result = await unsaveDailyIdea(`telegram:${account.telegramUserId}`, ideaId);
      console.info("daily_ideas_telegram_idea_unsaved", {
        requestId,
        actor: getTelegramActor(account.telegramUserId),
        ideaId,
    ...
    const result = await saveDailyIdea(`telegram:${account.telegramUserId}`, ideaId);
    if (!result.ok) return json({ error: "Idea not found", requestId }, 404);
    console.info("daily_ideas_telegram_idea_saved", {
      requestId,
      actor: getTelegramActor(account.telegramUserId),
      ideaId,
    ...
    console.error("daily_ideas_telegram_saves_write_failed", {
      requestId,
      actor: getTelegramActor(account.telegramUserId),
      ideaId,
      action,

...
function linkUrl(token: string) {
  ...
  const redirectPath = `/app/ideas?link=${encodeURIComponent(token)}`;
  try {
    const url = new URL("/sign-in", base);
    url.searchParams.set("redirect_url", redirectPath);
    return url.toString();
  } catch {
    return `https://www.gwapspot.com/sign-in?redirect_url=${encodeURIComponent(redirectPath)}`;
  }
}
...
export async function POST(request: Request) {
  ...
  if (!body || !account || (action !== "create" && action !== "status")) return json({ error: "Invalid link request", requestId }, 400);
  ...
  try {
    ...
    if (existing || action === "status") {
      return json({ contractVersion: DAILY_IDEAS_SERVICE_CONTRACT_VERSION, linked: Boolean(existing), identity: existing ? { gnsIdentity: existing.gnsIdentity, linkedAt: existing.linkedAt } : null });
    }
    ...
    console.info("daily_ideas_telegram_account_link_issued", { requestId, actor: getTelegramActor(account.telegramUserId) });
    return json({ contractVersion: DAILY_IDEAS_SERVICE_CONTRACT_VERSION, linked: false, link: { url: linkUrl(issued.token), expiresAt: issued.expiresAt } }, 201);
  } catch (error) {
    console.error("daily_ideas_telegram_account_link_failed", { requestId, actor: getTelegramActor(account.telegramUserId), name: error instanceof Error ? error.name : "Error" });

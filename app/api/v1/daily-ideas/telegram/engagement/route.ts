...
async function timezoneFor(telegramUserId: string) {
  const delivery = await getDailyIdeasDeliveryPreferences(telegramUserId);
  return delivery?.timezone || "UTC";
}
...
export async function GET(request: Request) {
  ...
  if (ideaId && !/^[A-Za-z0-9_-]{1,80}$/.test(ideaId)) {
    return json({ error: "Invalid idea", requestId }, 400);
  }
  ...
  try {
    ...
    const [engagement, idea] = await Promise.all([
      getDailyIdeasEngagement(`telegram:${account.telegramUserId}`, await timezoneFor(account.telegramUserId)),
    ...
    console.error("daily_ideas_telegram_engagement_read_failed", {
      requestId,
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const result = await recordDailyIdeasEngagement(`telegram:${account.telegramUserId}`, {
      eventId,
      kind,
      ideaId,
      rating,
      message,
      timezone: await timezoneFor(account.telegramUserId),
    });
    if (!result) return json({ error: "Invalid engagement event", requestId }, 400);
    console.info("daily_ideas_telegram_engagement_recorded", {
      requestId,
      actor: getTelegramActor(account.telegramUserId),
      kind,
      ...(ideaId ? { ideaId } : {}),
    ...
    console.error("daily_ideas_telegram_engagement_write_failed", {
      requestId,
      actor: account ? getTelegramActor(account.telegramUserId) : "invalid",
      kind,
      name: error instanceof Error ? error.name : "Error",
    });
    return json({ error: "Engagement data is temporarily unavailable", requestId }, 503);

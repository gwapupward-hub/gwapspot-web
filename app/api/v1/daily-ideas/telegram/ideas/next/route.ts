...
export async function GET() {
  ...
  return json({ ok: true, service: "daily-ideas-telegram-next-idea", contractVersion: DAILY_IDEAS_SERVICE_CONTRACT_VERSION, configured: configuration.configured, provider: configuration.provider, model: configuration.model });
}
...
export async function POST(request: Request) {
  ...
  if (!rate.allowed) return json({ error: "Daily idea request limit reached", requestId }, 429, { "Retry-After": String(rate.retryAfter) });
  try {
    const subject = await getDailyIdeasSubjectForTelegram(account.telegramUserId);
    const result = await getNextDailyIdea({ subject, category, mode, focus });
    console.info("daily_ideas_telegram_idea_delivered", { requestId, actor: getTelegramActor(account.telegramUserId), ideaId: result.idea.id, category: result.idea.category, mode, source: result.delivery.source });
    ...
    console.error("daily_ideas_telegram_idea_failed", { requestId, actor: getTelegramActor(account.telegramUserId), name: error instanceof Error ? error.name : "Error", providerStatus: providerError?.providerStatus ?? null, providerCode: providerError?.providerCode ?? null });

/**
 * Vendored from gwapupward-hub/ppv `sdk/src/canonical.ts` (@gwap/ppv-sdk).
 *
 * PPV canonicalization v1 is a frozen wire format: the bytes produced here are
 * the bytes a wallet commits to on chain, so this file must stay byte-for-byte
 * equivalent to the protocol SDK. It is vendored rather than installed because
 * the SDK is unpublished and lives in a private repository that a Vercel build
 * cannot authenticate against. `canonical.test.mjs` carries the same vectors
 * as the protocol repo so drift fails CI here instead of on chain.
 *
 * Do not edit this file to change behaviour. Change it upstream, re-run the
 * protocol test suite, then re-vendor.
 */

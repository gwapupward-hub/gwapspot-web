...
export type GwapOsState = { profile: GwapProfile; favorites: string[]; recent: RecentLaunch[]; ideas: DailyIdea[]; ideaProjects: IdeaProject[]; settings: GwapSettings };
...
export const MAX_GWAP_OS_STATE_BYTES = 48_000;
...
function normalizeProject(value: unknown): IdeaProject | null {
  ...
  const id = safeText(project.id, "", 80).replace(/[^A-Za-z0-9_-]/g, "");
  const ideaId = safeText(project.ideaId, "", 80).replace(/[^A-Za-z0-9_-]/g, "");
...
export const defaultGwapOsState: GwapOsState = { profile: { displayName: "GWAP Builder", handle: "gwap-builder", bio: "", primaryWallet: "", website: "", location: "", updatedAt: "" }, favorites: ["gns", "gwapscore", "isnad-sunnah"], recent: [], ideas: [], ideaProjects: [], settings: { compactMode: false, reduceMotion: false, bootAnimation: true, productUpdates: true, communityUpdates: true } };
export function createDefaultGwapOsState(): GwapOsState { return { profile: { ...defaultGwapOsState.profile }, favorites: [...defaultGwapOsState.favorites], recent: [], ideas: [], ideaProjects: [], settings: { ...defaultGwapOsState.settings } }; }
